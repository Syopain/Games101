...
static const int kPointNumber = 4;
std::vector<cv::Point2f> control_points;

void mouse_handler(int event, int x, int y, int flags, void *userdata) 
{
    if (event == cv::EVENT_LBUTTONDOWN && control_points.size() < kPointNumber)
    {
...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points, float t) 
{
    ...
    for (auto it = control_points.begin(); it != control_points.end() - 1; ++it) {
        next_points.push_back(*it * (1 - t) + *(it + 1) * t);
    }
...
int main() 
{
    ...
    while (key != 27) 
    {
        ...
        if (control_points.size() == kPointNumber)
        {
